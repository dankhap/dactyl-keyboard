  ...
  (let [top-wall (->> (cube (+ keyswitch-width 3) 1.5 plate-thickness)
        ...
        side-nub (->> (binding [*fn* 30] (cube 0.7 0.85 8.75));last number is nub size.  4.75 works for box
                      ...
                      (hull (->> (cube 1.5 2.75 1)
                                 (translate [(+ (/ 1.5 2) (/ keyswitch-width 2))
                                             0
                                             (/ plate-thickness 1.15)]))));2nd number controls slant height position
...
(def  thumb-left-wall-lower [(hull ;;top section
...
(def left-wall-dactyl
  (let [place case-place]
    (union (hull  (bottom-hull (place left-wall-column -0.18 (translate [1 -1 1] bottom-wall-sphere-bottom-back)))
...
(def wall-catter (hull (union left-wall-lower
                              (translate [20 0 0] left-wall-lower))))

(def full-oled-box (cube oled-depth oled-width  16))
(def inner-oled-box (cube (- oled-depth 1) (- oled-width 1) 15.8))

(defn place-oled [shape] (->> shape
                              (rotate (+ pi (/ pi 4) 0.18) [0 1 0])
                              (rotate (/ pi 15) [0 0 -1])
                              (rotate (/ pi 12) [1 0 0])
                              (translate [-47 28 21])))

(def left-wall-lower-hole (difference left-wall-lower (place-oled inner-oled-box)))

(def oled-box (difference
               full-oled-box
               (union
                (translate [-0.3 0 0] inner-oled-box)
                (translate [10 0 0] (cube 22 27 10)))))

(def bottom-oled-diff (let [s (->> (sphere 1)
                                   (with-fn 16))
                            round-oled (minkowski s oled-box)]
                        (union left-wall-lower-hole
                               (difference (place-oled round-oled)
                                           (translate [2.5 0 0] wall-catter)))))
     ...
     (apply union
		...
		;	 bottom-keys
			;bottom-keys2
             ...
             right-wall-lower
             back-wall-lower
             [bottom-oled-diff] ;[left-wall-lower]
			;(if (== row-size 0) left-wall-dactyl
		;	(if (== row-size 1) left-wall-lightcycle)) 
					  ...
					  ;(translate [0 0 -3])
                             ...
                             (for [xyz (range 1 10 1.8)]
                               (->> (union
                                     ...
                                     (translate [xyz (* -1 xyz) 0] thumb-front-wall-lower)))))) (translate [0 0 29] (cube 300 300 50))))
...
(def wrist-rest-build
  (->> (difference
        ...
        cut-bottom
        rest-case-cuts) (translate [20 -103 0])))
...
;;;;;######################################################
;;;;;
(def whole-front-wall (union (concat front-wall-lower
                                     thumb-front-wall-lower
                                     thumb-inside-lower)))
(def wrist-cutter (->> (hull (scale  [1 1 2] whole-front-wall)
                             (->> (union whole-front-wall)
                                  (translate [0 70 0])
                                  (scale [1 1 2])))
                       (rotate π [0 0 1])
                       (mirror [1 0 0])))
(def front-wrist (difference
                  (hull (->> (cube 90 40 30)
                             (translate [0 -10 15]))
                        (->> (cube 50 10 20)
                             (translate [0 -70 10])))
...
(spit "parts/wrist-wall.scad"
      (write-scad front-wrist))



(spit "parts/wrist-wall.scad"
      (write-scad (union front-wrist wrist-rest-build)))
...
(def bottom-case
  (scale [(- scale-bottom 0.0), scale-bottom, scale-bottom]
         (difference
          ...
          (if (== row-size 0) (->> (translate [0 0 0] usb-holder-hole)
                                   ;;(translate [0 0 0] trrs-holder-hole)
                                   ))
...
(spit "things/Dactyl-bottom-right-oled.scad"
